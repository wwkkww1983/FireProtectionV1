...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                   ...
                   // var expireDate = new Date(Date.now() + (100 * 1000));
                    var expireDate =  new Date(result.expires);
                    ...
                    abp.auth.setToken(result.tokenValue, expireDate);
                    callback();
                } else {
                    alert('Login failed !');
                }
        ...
        xhr.open('POST', '/api/services/Auth/Login/PostLogin', true);
        xhr.setRequestHeader('Abp.TenantId', tenantId);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.send("{" + "\"mobileNo\":\"" + usernameOrEmailAddress + "\"," + "\"password\":\"" + password + "\",smscode:\"\"}");
    ...
    abp.swagger.openAuthDialog = function (loginCallback) {
        ...
        createInput(modalUxContent, 'userName', 'Username or email address','input', '15888888888');
        createInput(modalUxContent, 'password', 'Password', 'password', '000000');
    ...
    function createInput(container, id, title, type,defaultValue) {
